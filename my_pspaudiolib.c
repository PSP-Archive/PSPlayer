...
static short __attribute__((aligned(64))) audio_sndbuf[PSP_NUM_AUDIO_CHANNELS][2][PSP_NUM_AUDIO_SAMPLES][2];
...
int sceAudioSetChannelDataLen(int ch, int rate);
...
/*		else
		{
			sceAudioSetChannelDataLen(AudioStatus[i].handle, 48000);
		}*/
	}
	if (failed) {
		for (i=0; i<PSP_NUM_AUDIO_CHANNELS; i++) {
	...
	for (i=0; i<PSP_NUM_AUDIO_CHANNELS; i++) {
		str[6]='0'+i;
		AudioStatus[i].threadhandle = sceKernelCreateThread(str,(void*)&AudioChannelThread,0x12,0x10000, THREAD_ATTR_VFPU,NULL);
